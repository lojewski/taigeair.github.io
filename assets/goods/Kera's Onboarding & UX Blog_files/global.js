(function($$, $){
  ...
  $(window).load(function() {
    ...
    var setCookie = function(key, value, expireDays) {
      // cookie(...) function is not globally defined, so we need to use this
      var expiry = new Date(Date.now() + 1000*3600*24*expireDays).toGMTString();
      document.cookie = key + "=" + value + "; expires=" + expiry + "; path=/";
    };
    // TOS Update Banner
    var $tosBanner = $('.j-tos_update_banner.j-desktop');
    var numTosShows = parseInt(document.cookie.replace(/(?:(?:^|.*;\s*)tos_update_banner_shows_desktop\s*\=\s*([^;]*).*$)|^.*$/, "$1"), 10);
    numTosShows = isNaN(numTosShows) ? 0 : numTosShows;
    var isMobile = $('#mobile-header').is(":visible");
    if ($tosBanner && $tosBanner.length > 0 && !isMobile &&
        numTosShows < 2) {
      var bannerHeight = $tosBanner.height();
      var $content = $('.sectionElements.container');
      $content = $content.length === 0 ? $('#list_page_share') : $content;
      var oldTop = parseInt($content.css('margin-top'), 10);
      var newTop = oldTop + bannerHeight;
      $content.css('margin-top', newTop);
      $tosBanner.slideDown();
      setCookie('tos_update_banner_shows_desktop', numTosShows + 1, 365);
      $tosBanner.find('.j-tos_close').on('click', function(e) {
        e.preventDefault();
        $tosBanner.hide();
        $content.css('margin-top', oldTop);
      });
    }
  });
